...
#include <string.h>
#include <stdio.h>
#include <ctype.h> //3lajal isalnum
#include <fcntl.h>
...
int replace(char* str, const char* s, const char* t, size_t max) {
    ...
    while (*pos) {
        char* found = strstr(pos, s);
        if (!found) {
            if (strlen(buffer) + strlen(pos) >= max) return -1;
            strcat(buffer, pos);
            break;
        }

        size_t prefix_len = found - pos;
        if (strlen(buffer) + prefix_len + len_t >= max) return -1;

        strncat(buffer, pos, prefix_len);
        strcat(buffer, t);
...
int substenv(char* str, size_t max) {
    ...
    for (size_t i = 0; str[i] != '\0'; i++) {
        ...
        if (str[i] == '$') {
            ...
            if (strlen(buffer) + strlen(val) >= max) return -1;
            strcat(buffer, val);
        } 
        else {
...
int parse_command_line(command_line_t* cmdl, const char* line) {
    ...
    while (cmdl->tokens[token_index] != NULL) {
        ...
        if (strcmp(token, "|") == 0) {
            int fds[2];
            if (pipe(fds) < 0) { perror("pipe"); close_fds(cmdl); return -1; }

           current_proc->stdout_fd = fds[1];
           add_fd(cmdl, fds[1]);

           processus_t* next = add_processus(cmdl, UNCONDITIONAL);
           next->stdin_fd = fds[0];
           add_fd(cmdl, fds[0]);
